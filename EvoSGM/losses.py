...
def get_sde_loss_fn(sde, train, eps=1e-5):
    ...
    def loss_fn(model, batch, condition=None):
        ...
        perturbed_data = torch.where(mask, coords_6d, perturbed_data)
...
def get_step_fn(sde, train, optimize_fn=None):
    ...
    def step_fn(state, batch, condition=None):
        ...
        if train:
            ...
            optimizer.zero_grad()  # 优化器梯度值清0
            loss = loss_fn(model, batch, condition)
            loss.backward()  # 根据loss计算参数梯度
            optimize_fn(
                optimizer, model.parameters(), step=state["step"]
            )  # 更新模型参数
            state["step"] += 1
            state["ema"].update(model.parameters())  # 使用ema平滑模型参数
        else:
            with torch.no_grad():
                ema = state["ema"]
