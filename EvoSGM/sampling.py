...
def get_pc_sampler(
    ...
    def pc_sampler(model, condition=None):
        ...
        with torch.no_grad():
            ...
            if condition is not None:
                for k, v in condition.items():
                    if k == "length":
                        ...
                        conditional_mask[:, :-1] ^= True  # NOTE
                        # print(conditional_mask[0][0])
                        # print(conditional_mask[0][-1])
                    elif k == "ss":
                        x[:, 5:8] = v
                        conditional_mask[:, 5:8] = True  # NOTE
                    elif k == "inpainting":
                        ...
                        conditional_mask[:, -1] = True  # NOTE
                        # print(f"Conditional mask: {conditional_mask[0][-1][0]}")
                        x = torch.where(
                            conditional_mask, coords_6d, x
                        )  # NOTE: True for coords_6d
                        # print(f"x: {x[0][-1][0]}")

            x_initial = x.detach().clone()  # x的副本
            ...
            # print(f"Conditional mask: {conditional_mask[0][0][10]}")
            # print(f"x_initial: {x_initial[0][-1]}")
            # print(f"x_mean: {x_mean[0][-1]}")
            # print(f"x: {x[0][-1]}")
