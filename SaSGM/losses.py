...
def block_dropout(coords_6d, ss_indices, block_dropout=0.2):
    for idx in range(len(ss_indices)):
        if ss_indices[idx] == "":
            continue  # no secondary structure annotation found
...
def get_sde_loss_fn(sde, train, eps=1e-5):
    ...
    def loss_fn(model, batch, condition=None, context=None):
        ...
        if "ss" in condition:
            coords_6d = block_dropout(
                coords_6d, batch["ss_indices"]
            )  # Dropout block adjacencies

        score_fn = mutils.get_score_fn(sde, model, train=train)
        t = (
            torch.rand(coords_6d.shape[0], device=coords_6d.device) * (sde.T - eps)
            + eps
        )
        ...
        perturbed_data = torch.where(mask, coords_6d, perturbed_data)  # NOTE

        score = score_fn(perturbed_data, t, context)

        losses = torch.square(score * std[:, None, None, None] + z) * mask
        losses = torch.sum(losses.reshape(losses.shape[0], -1), dim=-1)
        losses = losses / (
            num_elem + 1e-8
        )  # 1e-8 added to prevent nan when num_elem TODO: Fix masking to prevent this
...
def get_step_fn(sde, train, optimize_fn=None):
    ...
    # 外层函数的意义在于对于train(T/F)和sde选取不同的loss_fn
    loss_fn = get_sde_loss_fn(sde, train)

    def step_fn(state, batch, condition=None, context=None):
        ...
        if train:
            ...
            optimizer.zero_grad()
            loss = loss_fn(model, batch, condition, context)
            loss.backward()
            optimize_fn(
                optimizer, model.parameters(), step=state["step"]
            )  # 更新模型参数
            state["step"] += 1
