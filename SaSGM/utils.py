...
def get_condition_from_batch(config, batch, mask_info=None, context=None):
    ...
    for c in config.model.condition:
        if c == "length":
            ...
            if mask_info is not None:
                batch_masked = selected_mask_batch(batch, mask_info, config)  # NOTE
            else:
...
def get_conditions_from_pdb(
    pdb, config, chain="A", mask_info=None, batch_size=8, context_path=None
):
    ...
    ds = ProteinDataset(
        ...
        config.data.max_res_num,
        ss_constraints,
        context_path=context_path,  # NOTE
    )

    dl = torch.utils.data.DataLoader(
        [ds[0]] * batch_size,  # NOTE
        batch_size=batch_size,
        collate_fn=PaddingCollate(config.data.max_res_num, use_context=True),
    )
    batch, context_batch = next(iter(dl))

    return get_condition_from_batch(
        config, batch, mask_info=mask_info, context=context_batch
    )
...
def get_model_fn(model, train=False):
    ...
    def model_fn(x, labels, context=None):
        ...
        if not train:
            model.eval()
            return model(x, labels, context)  # NOTE: whether eval use context
        else:
            model.train()
