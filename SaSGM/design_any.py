...
def sampling_6d(
    ...
    workdir = Path("benchmark", "output_bbs", "sasgm", pdb_name)
    # if protein_sgm:
    ...
    state["ema"].store(state["model"].parameters())  # store到collected_params
    state["ema"].copy_to(state["model"].parameters())  # 从shadow_params copy
    ...
    for _ in tqdm(range(n_iter)):
        ...
        sample, n = sampling_fn(state["model"], condition)  # NOTE
...
def rosetta_minimization(
    ...
    outPath = Path("benchmark", "output_bbs", "sasgm", tag)
...
def main():
    ...
    parser.add_argument(
        "--cpt_path",
        type=str,
        default="/lustre/home/acct-stu/stu006/zijiancheng/SaSGM/checkpoints/sasgm/cond_length_inpainting.pth",
    )
